...
class CityDriveDQN(gym.Env, EzPickle):
    """
    ## Description
    ...
    * `continuous=True` specifies if the agent has continuous (true) or discrete (false) actions.
    ...
    def step(self, action: np.ndarray | int):
        ...
        # assume self.state['pixels'] is the numeric array
        return np.array(self.state['pixels'], dtype=np.float32), step_reward, terminated, truncated, info
